...
class Model: 
	...
	def setupCTC(self):
		...
		if self.decoderType == DecoderType.BestPath:
			...
			word_beam_search_module = tf.load_op_library('newSimpleHTR/src/TFWordBeamSearch.so')

			# prepare information about language (dictionary, characters in dataset, characters forming words) 
			chars = str().join(self.charList)
			wordChars = open('newSimpleHTR/model/wordCharList.txt').read().splitlines()[0]
			corpus = open('newSimpleHTR/data/corpus.txt').read()
	...
	def save(self):
		"save model to file"
		self.snapID += 1
		self.saver.save(self.sess, 'newSimpleHTR/model/snapshot', global_step=self.snapID)
